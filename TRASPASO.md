...
`apagado`, y **media hora después entró un pedido de PedidosYa con ese producto**.
...
Call log: - waiting for get_by_text("Tarta de choclo", exact=True).first.locat...)
```
...
`/api/diagnostico?plataforma=pedidosya&nombre=Tarta de choclo` devuelve el HTML crudo de
la fila, y `/api/esqueleto?plataforma=pedidosya` el árbol del DOM. Lo que hay
que contestar es **cuántos elementos matchean `get_by_text("Tarta de choclo",
...
- **"Tarta de verdura chica" (PedidosYa) NO es "Tarta de verdura individual"
  (Rappi).** Son platos distintos.
- Los únicos 3 tartas activos en PedidosYa son los "chica", vinculados a
  mano con los de Rappi.
- **La Mixta y los ~18 productos que están solo en Rappi no se cargan a
